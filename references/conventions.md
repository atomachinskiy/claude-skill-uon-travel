...
## 14. Карточка туриста — особенности при чтении и записи

`POST /user/create` и `POST /user/update/{id}` принимают **~60 полей**: ФИО (+ латиница для виз), демография, контакты, соцсети, мессенджеры (vk/fb/ok/telegram/max/whatsapp/viber/instagram), оба паспорта (загран + гражданский), свидетельство о рождении, юр. реквизиты (для u_type=2), связи (manager, office, nationality, referrer, labels), бонусы.

CLI закрывает топ-40 через именованные флаги, остальное — через `-F`.

**Особенности при сохранении:**

- `u_sex="м"` нормализуется в `1`, `"ж"` в `2` (внутренний код).
- `u_zagran_number="73 1234567"` (с пробелом) сохраняется как `"731234567"` (пробелы вырезаются).
- То же для `u_passport_number` — серия и номер слипаются.
- Даты `u_birthday`, `u_zagran_given`, `u_passport_date` сохраняются как datetime с дописанным `00:00`. При чтении приходят как `"YYYY-MM-DD 00:00"`.
- `u_labels` — CSV-строка имён меток (`"VIP,Семья,Активный отдых"`). Имена должны существовать в `labels list` — несуществующие игнорируются.

**Метки (labels):**

```bash
uon labels list   # GET /user-label — id, name, color
```

Создание/удаление меток через API **не поддерживается** — только в UI кабинета (Настройки → Метки).

**Кто привёл (referrer):**

`--referrer-id N` мапится в поле `u_source` — это ID другого туриста, который привёл этого. Полезно для реферальной аналитики (через `stats.py` можно посчитать вклад «приведённых»).

**Юр. лицо:**

Передаём `--type 2` плюс `--company`, `--position`, `--inn`, `--kpp`, `--ogrn`. Появляется отдельный набор полей в кабинете. Финансы (банк, расчётный счёт, БИК, ОКПО) — через `-F u_finance_bank=...`.

**Турагентство:**

`--type 3` — для агентств-партнёров (для туроператоров, работающих с агентской сетью).
